...
class postscript_tables_core_999250
{
    var $oDBUpgrade;
    var $oDbh;
    ...
    function execute_constructive($aParams)
    {
        ...
        $this->_log('**********postscript_tables_core_999250**********');
        $this->_logActual();
        return true;
    }
    ...
    function _logActual()
    {
        ...
        if (in_array($prefix.'astro', $aExistingTables))
        {
            ...
            {
                $this->_log('changes_tables_core_999250::TEST A : added autoincrement field for table '.$prefix.'astro defined as:');
                $this->_log(print_r($aDef['tables']['astro']['fields']['auto_field'],true));
            }
            else
            {
                $this->_log('changes_tables_core_999250::TEST A : failed to add autoincrement field for table '.$prefix.'astro');
            }
        }
    }
}
