...
class postscript_tables_core_999450
{
    var $oDBUpgrade;
    var $oDbh;
    ...
    function execute_constructive($aParams)
    {
        ...
        $this->_log('**********postscript_tables_core_999450**********');
        $this->_logActual();
        return true;
    }
    ...
    function _logActual()
    {
        ...
        if (!isset($aDef['tables']['astro']))
        {
            $this->_log('changes_tables_core_999450::TEST A : rename table '.$prefix.'astro');
        }
        else
        {
            $this->_log('changes_tables_core_999450::TEST A : failed to rename table '.$prefix.'astro');
        }
        $aDef = $this->oDBUpgrade->_getDefinitionFromDatabase('klapaucius');
        if (isset($aDef['tables']['klapaucius']))
        {
            $this->_log('changes_tables_core_999450::TEST A : renamed table to '.$prefix.'klapaucius defined as:');
            $this->_log(print_r($aDef['tables']['klapaucius'],true));
        }
        else
        {
            $this->_log('changes_tables_core_999450::TEST A : failed to rename table to'.$prefix.'klapaucius');
        }
    }
}
