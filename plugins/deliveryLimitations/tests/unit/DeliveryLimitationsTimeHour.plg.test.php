...
class Plugins_TestOfPlugins_DeliveryLimitations_Time_Hour extends UnitTestCase
{
     ...
     */
    function Plugins_TestOfPlugins_DeliveryLimitations_Time_Hour()
    {
        $this->UnitTestCase();
    }
    ...
    {
        $oPlugin = &MAX_Plugin::factory('deliveryLimitations', 'Time', 'Hour');
        $result = $oPlugin->_getSqlLimitation('==', '0');
        $this->assertEqual($result, "LOWER(HOUR(date_time)) IN ('0')");
        $result = $oPlugin->_getSqlLimitation('!=', '0,3,5');
        $this->assertEqual($result, "LOWER(HOUR(date_time)) NOT IN ('0','3','5')");
    }
