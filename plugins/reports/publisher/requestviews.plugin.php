...
+---------------------------------------------------------------------------+
$Id: requestviews.plugin.php 6108 2006-11-24 11:34:58Z andrew@m3.net $
*/
...
class Plugins_Reports_Publisher_Requestviews extends Plugins_Reports {
    ...
    function info()
    {
        ...
        $plugininfo = array (
            'plugin-name'           => MAX_Plugin_Translation::translate('Request / Views Analysis', $this->module, $this->package),
            'plugin-description'    => MAX_Plugin_Translation::translate('The table is comparison between total ad requests and total actual views by date.', $this->module, $this->package),
    ...
    function execute($affiliateid, $startdate, $enddate)
    {
    	...
    	$reportName = $GLOBALS['strPublisherRequestViewsAnalysisReport'];
        ...
        $worksheet = generateXLSHeader($workbook, $reportName, $startDate, $endDate,
                                       $affiliateid, $GLOBALS['strPublisherRequestViewsAnalysisDescription']);
    	...
    	if (phpAds_dbNumRows($res)) {
        	...
        	foreach ($dataSorted as $block) {
        	   ...
        	   foreach ($block as $rowId => $row) {
        	       $worksheet->write($currentRow, $columnStart, $row['day']);
    ...
    function prepareData(& $data)
    {
        ...
        foreach ($data as $row) {
            ...
            $totalBlock[$row['day']]['day'] = $row['day'];
            $totalBlock[$row['day']]['priority'] = 't';
            $totalBlock[$row['day']]['requests']    += $row['requests'];
            $totalBlock[$row['day']]['impressions'] += $row['impressions'];
            $totalBlock[$row['day']]['clicks']      += $row['clicks'];
            $totalBlock[$row['day']]['conversions'] += $row['conversions'];
            $blockIndex++;
        }
        ...
        foreach ($retArray as $blockId => $block) {
            // ksort($retArray[$blockId]); //already sorted in db
