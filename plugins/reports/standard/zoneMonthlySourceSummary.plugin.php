...
+---------------------------------------------------------------------------+
$Id:zoneMonthlySourceSummary.plugin.php 114 2006-07-19 12:15:00Z james.easterby@m3.net $
*/
...
class Plugins_Reports_Standard_ZoneMonthlySourceSummary extends Plugins_ExcelReports
{
    /* @var int */
    var $_start_month;
    ...
    function initInfo()
    {
        $this->_name = 'Zone Monthly Source Summary';
        $this->_description = 'This report shows ad impressions and clicks by zone and source for the specified period.';
        $this->_category = 'standard';
        $this->_categoryName = 'Standard Reports';
        $this->_author = 'James Easterby';

        if ($this->hasZoneSourceForecasts()) {
    ...
    function hasZoneSourceForecasts()
    {
        ...
        $aParams['zone_inventory_forecast_type'] = 4;
        $aZones = Admin_DA::getZones($aParams);
        $hasZoneSourceForecasts = (sizeof($aZones) > 0);
        return $hasZoneSourceForecasts;
    }
    ...
    function getDefaults()
    {
        ...
        $aImport['zone'] = array(
            ...
            'filter' => 'zone-inventory-source-indexed',
    ...
    function execute($zoneScope, $start_month, $end_month, $reslimit)
    {
        ...
        $this->addMonthlySourceEffectivenessSheet();
    ...
    /**
     * Adds a worksheet to the report containing views and clicks for each day/source.
     *
     * @param Array $aReportParameters The values used to generate the report as a whole
     * @param Array $reslimit The number of rows to return in descending order of impression count.
     */
    function addMonthlySourceEffectivenessSheet()
    {
        $headers = array(
            'Month' => 'date',
            'Source' => 'text',
        ...
        $months_data = $this->dal->getEffectivenessForZoneByMonthSource($this->_zoneId, $this->_start_month, $this->_end_month, $this->_reslimit);
        $months_display = $this->prepareMonthlySourceEffectivenessForDisplay($months_data);

        $this->createSubReport('Monthly Source Breakdown', $headers, $months_display);
    }
