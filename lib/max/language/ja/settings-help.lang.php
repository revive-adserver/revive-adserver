...
$GLOBALS['phpAds_hlp_dbhost'] = "　　　　　　　 接続したい".phpAds_dbmsname."データベースサーバのホスト名を入力してください。
		";
...
$GLOBALS['phpAds_hlp_dbuser'] = "                アクセスしたい{$PRODUCT_NAME}用".phpAds_dbmsname."データベースサーバのユーザ名を入力してください。
		";

$GLOBALS['phpAds_hlp_dbpassword'] = "                アクセスしたい{$PRODUCT_NAME}用".phpAds_dbmsname."データベースサーバのパスワードを入力してください。
		";

$GLOBALS['phpAds_hlp_dbname'] = "{$PRODUCT_NAME}がデータ保存用に使用するデータベース名を入力してください。
		データベースサーバ上にあらかじめデータベースを作成してください。{$PRODUCT_NAME}は、指定したデータベースが存在しない場合、データベースを<b>自動作成しません</b>。
		";

$GLOBALS['phpAds_hlp_persistent_connections'] = "                パーシスタント接続を使用すると、{$PRODUCT_NAME}をかなりスピーディに動作させ、サーバ負荷を低減できます。
		しかし欠点もあります。通常接続を使用する場合と比較して、多くの訪問者が訪れるサイトではサーバ負荷が上昇します。
		通常接続にすべきかパーシスタント接続にすべきかは、訪問者数とハードウェアスペックに依存します。
		もし、{$PRODUCT_NAME}が多くのサーバリソースを食っている場合、この設定を最初に見直してください。
		";



$GLOBALS['phpAds_hlp_compatibility_mode'] = "                もし、{$PRODUCT_NAME}とサードパーティ製品との統合に問題がある場合、データベースの互換モードの変更が
		役に立つかもしれません。ローカルモード呼出を使用し、データベース互換性が'ON'の場合、{$PRODUCT_NAME}は
		データベースの互換性を維持するようにデータベース接続状態を保ちます。
		データベース互換モードを使用すると、若干動作が遅くなるので、デフォルトでは'OFF'にしています。
		";


$GLOBALS['phpAds_hlp_table_prefix'] = "                もし、{$PRODUCT_NAME}がデータベースを複数のソフトウェアと共有して使用する場合、テーブル名にプリフィックスを
		追加してください。もし、同一データベースに複数の{$PRODUCT_NAME}をインストールしている場合も同じように
		異なるプリフィックスを追加して、各{$PRODUCT_NAME}がユニークになるようにしてください。
		";

$GLOBALS['phpAds_hlp_table_type'] = "        ".phpAds_dbmsname."は、複数のテーブルタイプをサポートしています。各々のテーブルタイプは異なる特性を有し、
		いくつかのタイプは{$PRODUCT_NAME}をかなりスピードアップさせます。MyISAMは、".phpAds_dbmsname."		のデフォルトテーブルタイプで、のすべてのインストールで利用可能です。他のテーブルタイプは、多くのサーバで利用できない
		ケースがあります。";

$GLOBALS['phpAds_hlp_url_prefix'] = "        {$PRODUCT_NAME}は、Webサーバが正しく動作するように、Webサーバの場所を知っている必要があります。
        {$PRODUCT_NAME}のインストールディレクトリを示すURLを入力してください。
                入力例）<i>http://www.your-url.com/{$PRODUCT_NAME}</i>
		";

$GLOBALS['phpAds_hlp_ssl_url_prefix'] = "        {$PRODUCT_NAME}は、Webサーバが正しく動作するように、Webサーバの場所を知っている必要があります。
                時として、 SSLプリフィックスは、標準URLのプリフィックスと異なります。
        {$PRODUCT_NAME}のインストールディレクトリを示すURLを入力してください。
                入力例）<i>https://www.your-url.com/{$PRODUCT_NAME}</i>.
		";

$GLOBALS['phpAds_hlp_my_header'] =
$GLOBALS['phpAds_hlp_my_footer'] = "                管理者インタフェース内の各々のページがヘッダーやフッターを持つように、各ファイルへのパスを入力してください。
                入力例） /home/login/www/header.htm
                これらのファイルには、テキストもしくはHTMLを入力できます（HTML使用時は、<body>タグと<html>タグは
                不要です)。
		";

$GLOBALS['phpAds_hlp_my_logo'] = "                デフォルトロゴに代えて表示したいカスタムロゴのファイル名を入力してください。
                ロゴファイル名を入力する前に、admin/imagesディレクトリに必ずロゴファイルをアップロードしてください。
        ";


$GLOBALS['phpAds_hlp_gui_header_foreground_color'] = "                タブ、サーチバーおよび強調文字で使用する文字色を入力してください。
        ";


$GLOBALS['phpAds_hlp_gui_header_background_color'] = "                ヘッダー背景に使用する背景色を入力してください。
        ";

$GLOBALS['phpAds_hlp_gui_header_active_tab_color'] = "                選択中のメインタブに使用する背景色を入力してください。
        ";

$GLOBALS['phpAds_hlp_gui_header_text_color'] = "                ヘッダー内のテキストに使用する文字色を入力してください。
        ";
...
$GLOBALS['phpAds_hlp_company_name'] = "このメールは {$PRODUCT_NAME} により送信されています。 ";
...
$GLOBALS['phpAds_hlp_warn_client'] = "もしキャンペーンが１つでも以下の状態の場合、{$PRODUCT_NAME} は広告主宛てにメールを送信することが出来ます。";

$GLOBALS['phpAds_hlp_qmail_patch'] = "あるバージョンのQmailにはバグがあります。たとえばヘッダ情報を全て表示します。この設定をオンにした場合、{$PRODUCT_NAME} はQmailに対応したフォーマットでメールを送信します。";

$GLOBALS['phpAds_hlp_warn_limit'] = "{$PRODUCT_NAME} が警告を送信し始める閾値は100です。";
