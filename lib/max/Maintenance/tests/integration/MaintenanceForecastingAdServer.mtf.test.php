...
// pgsql execution time before refactor: 1279.4s
...
class Maintenance_TestOfMaintenanceForecastingAdServer extends UnitTestCase
{
    ...
    function _prepareStatements()
    {
        ...
        $table = $oDbh->quoteIdentifier($aConf['table']['prefix'].$aConf['table']['channel'], true);
        $query = "
            INSERT INTO
                {$table}
                (
        ...
        $table = $oDbh->quoteIdentifier($aConf['table']['prefix'].$aConf['table']['acls_channel'], true);
        $query = "
            INSERT INTO
                {$table}
                (
    ...
    function _insertZone()
    {
        $aConf = $GLOBALS['_MAX']['CONF'];
        $oDbh = OA_DB::singleton();
        $table = $oDbh->quoteIdentifier($aConf['table']['prefix'].$aConf['table']['zones'], true);
        $query = "
            INSERT INTO
                {$table}
                (
    ...
    function _buildQuerySelect($columns, $table)
    {
        $aConf = &$GLOBALS['_MAX']['CONF'];
        $oDbh = &OA_DB::singleton();
        $table = $oDbh->quoteIdentifier($aConf['table']['prefix'].$aConf['table'][$table], true);
        return "
            SELECT
                {$columns}
            FROM
...
//        $aRow = $rc->fetchRow();
//        $this->assertEqual($aRow['number'], 1);
//        $table = $oDbh->quoteIdentifier($aConf['table']['prefix'].$aConf['table'][''], true);
//        $query = $this->_buildQuerySelect("day,channel_id,zone_id,actual_impressions",'data_summary_channel_daily');
//        $rc = $oDbh->query($query);
//        $aRow = $rc->fetchRow();
//        $this->assertEqual($aRow['day'], '2006-06-14');
