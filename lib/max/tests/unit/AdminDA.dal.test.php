...
class Admin_DaTest extends DalUnitTestCase
{
    ...
    public function getLastRecordInserted($tableName, $tableIndexField)
    {
        $conf = $GLOBALS['_MAX']['CONF'];
        $tableName = $conf['table']['prefix'] . $tableName;
        $sql = "SELECT MAX($tableIndexField) AS max FROM " . $dbh->quoteIdentifier($tableName, true);
        $result = $dbh->getRow($sql, [], DB_FETCHMODE_ASSOC);

        return (int) $result['max'];
    }

    public function _haveStats($array)
    {
        if (!is_array($array)) {
            return false;
        }
    ...
    public function test_getColumns()
    {
        ...
        foreach ($entities as $entity => $hash) {
            $ret = SqlBuilder::_getColumns($entity, [], true);
    ...
    public function test_getTables()
    {
        require MAX_PATH . '/tests/data/data.entities.php';
        foreach ($entities as $entity => $hash) {
            $ret = SqlBuilder::_getTables($entity, []);
            $this->assertTrue(is_array($ret));
    ...
    public function test_getLimitations()
    {
        require MAX_PATH . '/tests/data/data.entities.php';
        foreach ($entities as $entity => $hash) {
    ...
    // | placementZones                        |
    // +---------------------------------------+
    public function testAddPlacementZone()
    {
        // starting the transaction or subtransaction
        TestEnv::startTransaction();

        $ret = Admin_DA::addPlacementZone(['zone_id' => 1, 'placement_id' => 2]);
        $this->assertTrue(is_int($ret));
        $this->assertTrue($ret > 0);
    ...
    public function testPlacementZones()
    {
        TestEnv::startTransaction();

        $ret = Admin_DA::addPlacementZone(['zone_id' => rand(1, 999), 'placement_id' => rand(1, 999)]);
        $this->assertTrue(is_int($ret));
        $this->assertTrue($ret > 0);

        $retVar = Admin_DA::getPlacementZones(['placement_zone_assoc_id' => $ret]);
    ...
    public function testGetDeliveryLimitations()
    {
        ...
        $ret = Admin_DA::getDeliveryLimitations(['zone_id' => 1, 'placement_id' => 2]);
        $this->assertTrue(is_array($ret));
    ...
    {
        $variableId = Admin_DA::addVariable([
    ...
    public function testDuplicateTracker()
    {
        TestEnv::startTransaction();

        $trackerId = Admin_DA::addTracker([
            'trackername' => 'foo',
            'description' => 'bar',
            'clientid' => 0,
            'appendcode' => 'a',
    ...
    public function testDuplicatePlacement()
    {
        TestEnv::startTransaction();

        $placementId = Admin_DA::addPlacement([
            'campaignname' => 'foo',
            'clientid' => 0,
            'views' => rand(1, 9999),
            'clicks' => rand(1, 9999),
    ...
    public function testGetZones()
    {
        ...
        $aZone2 = Admin_DA::getZones(['zone_id' => $ret]);
        /*
        Array
        (
            [80] => Array
    ...
    public function testDuplicateZone()
    {
        $conf = $GLOBALS['_MAX']['CONF'];
        $dbh = OA_DB::singleton();
        $id = $this->newZone();
    ...
    public function testAddCategory()
    {
        ...
        $ret = Admin_DA::addCategory(['name' => $name->create()]);
        $this->assertTrue(is_int($ret));
        $this->assertTrue($ret > 0);
    ...
    public function testAddAdCategory()
    {
        TestEnv::startTransaction();

        $ret = Admin_DA::addAdCategory(['ad_id' => 1, 'category_id' => 2]);
        $this->assertTrue(is_int($ret));
        $this->assertTrue($ret > 0);
    ...
    public function testAddAdZone()
    {
        $this->_generateStats();
        $ret = Admin_DA::addAdZone(['zone_id' => $this->zoneId, 'ad_id' => $this->bannerId]);
        $this->assertTrue(is_int($ret));
        $this->assertTrue($ret > 0);
    }

    public function testAddAdZoneCrossAgency()
    {
        $this->_generateStats();
        $ret = Admin_DA::addAdZone(['zone_id' => $this->zoneId3, 'ad_id' => $this->bannerId]);
        $this->assertisA($ret, 'PEAR_Error');
    }

    public function testCheckBannerType()
    {
        $this->_generateStats();
        $ret = Admin_DA::addAdZone(['zone_id' => $this->zoneId, 'ad_id' => $this->bannerId]);
        $this->assertTrue(is_int($ret));
        ...
        $ret = Admin_DA::addAdZone(['zone_id' => $this->zoneId, 'ad_id' => $this->bannerId2]);
        $this->assertTrue(PEAR::isError($ret));

        //  attempt to add a Text banner to a Email/Newsletter zone which should fail
        $ret = Admin_DA::addAdZone(['zone_id' => $this->zoneId2, 'ad_id' => $this->bannerId]);
        $this->assertTrue(PEAR::isError($ret));
    }

    public function testCheckBannerZoneAdAssoc()
    {
    ...
    public function testdeleteAdZones()
    {
        $this->_generateStats();
        $ret = Admin_DA::addAdZone(['zone_id' => $this->zoneId, 'ad_id' => $this->bannerId]);
        $this->assertTrue(is_int($ret));
        $this->assertTrue($ret > 0);
        Admin_DA::deleteAdZones(['zone_id' => $this->zoneId, 'ad_id' => $this->bannerId]);
    }
    ...
    public function testReturnByColumnType()
    {
        $id = Admin_DA::addPlacement([
            'campaignname' => 'foo',
            'clientid' => 0,
    ...
    public function testFromCacheGetPlacements()
    {
        $id = Admin_DA::addPlacement([
            'campaignname' => 'foo',
            'clientid' => 0,
    ...
    public function testFromCacheGetPlacement()
    {
        $id = Admin_DA::addPlacement([
            'campaignname' => 'foo',
            'clientid' => 0,
    ...
    // |                                       |
    // +---------------------------------------+
    public function testGetPlacementsStats()
    {
        $this->_generateStats();
        $ret = Admin_DA::getPlacementsStats(['advertiser_id' => $this->clientId]);
    ...
    public function testGetPlacementsStatsTwoParams()
    {
        $this->_generateStats();
        $ret = Admin_DA::getPlacementsStats(['advertiser_id' => $this->clientId, 'placement_id' => $this->campaignId]);
        $this->assertTrue($this->_haveStats($ret));
    }

    public function testGetAdvertisersStats()
    {
        $this->_generateStats();
        $ret = Admin_DA::getAdvertisersStats(['agency_id' => $this->agencyId]);
        $this->assertTrue($this->_haveStats($ret));
    }

    public function testGetPublishersStats()
    {
        $this->_generateStats();
        $ret = Admin_DA::getPublishersStats(['agency_id' => $this->agencyId]);
        $this->assertTrue($this->_haveStats($ret));
    }

    public function testGetZonesStats()
    {
        $this->_generateStats();
        $ret = Admin_DA::getZonesStats(['publisher_id' => $this->affiliateId]);
        $this->assertTrue($this->_haveStats($ret));
    }

    public function testGetAdsStats()
    {
        $this->_generateStats();
        $ret = Admin_DA::getAdsStats(['placement_id' => $this->campaignId]);
        $this->assertTrue($this->_haveStats($ret));
    }
    ...
    public function testFromCacheGetPlacementsStats()
    {
        $this->_generateStats();

        $ret = Admin_DA::fromCache('getPlacementsStats', ['advertiser_id' => $this->clientId]);
        $this->assertTrue($this->_haveStats($ret));
    }

    public function testFromCacheGetAdvertisersStats()
    {
        $this->_generateStats();

        $res = Admin_DA::fromCache('getAdvertisersStats', ['agency_id' => $this->agencyId]);
        $this->assertTrue(is_array($res));
        $this->assertTrue(count($res));
    }

    public function testFromCacheGetPublishersStats()
    {
        $this->_generateStats();

        $res = Admin_DA::fromCache('getPublishersStats', ['agency_id' => $this->agencyId]);
        $this->assertTrue(is_array($res));
        $this->assertTrue(count($res));
    }

    public function testFromCacheGetZonesStats()
    {
        $this->_generateStats();

        $res = Admin_DA::fromCache('getZonesStats', ['publisher_id' => $this->affiliateId]);
        $this->assertTrue(is_array($res));
        $this->assertTrue(count($res));
    }

    public function testFromCacheGetAdsStats()
    {
        $this->_generateStats();

        $res = Admin_DA::fromCache('getAdsStats', ['placement_id' => $this->campaignId]);
        $this->assertTrue(is_array($res));
        $this->assertTrue(count($res));
    }

    public function _generateStats()
    {
        $this->aIds = TestEnv::loadData('data_summary_ad_hourly_001');
        $this->agencyId = $this->aIds['agency'][1];
        $this->agencyId2 = $this->aIds['agency'][2];
        ...
        $this->affiliateId2 = $this->aIds['affiliates'][2];
        $this->zoneId = $this->aIds['zones'][1];
        $this->zoneId2 = $this->aIds['zones'][2];
        $this->zoneId3 = $this->aIds['zones'][3];
    }
}
