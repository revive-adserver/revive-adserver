...
class Admin_DaTest extends DalUnitTestCase
{
    ...
    function test_GetAgenciesCampaignStats()
    {
//        TestEnv::startTransaction();
        ...
        $cAgencies = 2;
        $cClientsPerAgency = 1;
        $cClients = $cAgencies * $cClientsPerAgency;
        $cCampaignsPerClient = 2;
        $cCampaigns = $cClients * $cCampaignsPerClient;
        $cBannersPerCampaign = 1;
        $cBanners = $cCampaigns * $cBannersPerCampaign;
        
        $dataGenerator = new DataGenerator();
        $aAgencyIds = $dataGenerator->generate('agency', $cAgencies);
        
        $dataGenerator->setData('clients', array('agencyid' => $aAgencyIds));
        $aClientIds = $dataGenerator->generate('clients', $cClients);
        
        $dataGenerator->setData('campaigns', array('clientid' => $aClientIds));
        $aCampaignIds = $dataGenerator->generate('campaigns', $cCampaigns);
        
        $dataGenerator->setData('banners', array('campaignid' => $aCampaignIds));
        $aBannerIds = $dataGenerator->generate('banners', $cBanners);
...
//        $result = $dbh->exec($sql);
        ...
        $summaryTable = $conf['table']['prefix'] . 'data_summary_ad_hourly';
        $dbh = &OA_DB::singleton();
        $stmt = &$dbh->prepare(
            "INSERT INTO $summaryTable
            (day, hour, ad_id, creative_id, zone_id, requests, impressions, clicks, conversions, total_basket_value)
            VALUES (?, ?, ?, 0, 0, 0, 100, 100, 0, NULL)");
        // define stats
...
//            VALUES(?, ?, ?, 0,0,0,100,100,0,NULL)";
        ...
        foreach ($aDays as $day => $aHours) {
            foreach ($aHours as $hour => $aAdIds) {
                foreach($aAdIds as $adId) {
                        $stmt->bindParam(0, $day);
                        $stmt->bindParam(1, $hour);
                        $stmt->bindParam(2, $adId);
                        $success = $stmt->execute();
                        $this->assertFalse(PEAR::isError($success));
                    };
            }
        }
        
        // test 1 - get data for a single hour for a single agency
        $aAgencies = array($aAgencyIds[0]);
        ...
        $aData = Admin_DA::getAgenciesCampaignStats($aAgencyIds, $oStart, $oEnd);
        ...
        $this->assertTrue(count($aData) == $cAgencies);
