 ...
 */
class Maintenance_TestOfMaintenancePriorityAdServerBannerLimitationsTzDaily extends UnitTestCase
{
     ...
     * Test Basis:
     *
     * - One campaign, with no start or end date.
     * - Daily impressions limit of 23,000 impressions (i.e. 1,000 per hour
     *     required on average, running 23 hours per day).
    ...
    public function testCampaignWithTimezones()
    {
        ...
        $doCampaigns->activate_time = OX_DATAOBJECT_NULL;
        $doCampaigns->expire_time = OX_DATAOBJECT_NULL;
        $doCampaigns->priority = 10;
        $doCampaigns->target_impression = 23000;
        ...
        for ($counter = 1; $counter <= 48; $counter++) {
            ...
            $oGetRequiredAdImpressionsDaily->run();
            ...
            $doData_intermediate_ad->date_time = $aDates['start']->format('%Y-%m-%d %H:%M:%S');
