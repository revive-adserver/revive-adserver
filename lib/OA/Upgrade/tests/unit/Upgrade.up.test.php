...
{
    public function _rollbackPutAdmin()
    {
        parent::_rollbackPutAdmin();
    }
}
...
class Test_OA_Upgrade extends UnitTestCase
{
    ...
    public function _writeUpgradePackagesArray()
    {
...
//    }
     ...
     *
     */
    public function test_getUpgradePackagesList()
    {
    ...
    public function test_doBackups_and_pickupNoBackupsFile()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function test_writeRecoveryFile()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function test_seekRecoveryFile()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function test_pickupRecoveryFile()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function test_getOriginalApplicationVersion()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function testGetPostUpgradeTasks()
    {
        // create upgrade file
        $oUpgrade = new OA_Upgrade();
        ...
        $this->assertEqual($result, ['Test_1', 'Test_2', 'Test_3']);
    ...
    public function testRunPostUpgradeTask()
    {
        $oUpgrade = new OA_Upgrade();
        $oUpgrade->upgradePath = MAX_PATH . '/lib/OA/Upgrade/tests/data/';
        Mock::generatePartial(
        ...
        $oLogger1->expectOnce('logError', ['Error from Test_1']);
        $oLogger1->expectAt(0, 'logOnly', ['attempting to include file ' . $file]);
        $oLogger1->expectAt(1, 'logOnly', ['Message from Test_1']);
        $oLogger1->expectAt(2, 'logOnly', ['executed file ' . $file]);
        ...
        $oLogger2->expectAt(0, 'logOnly', ['attempting to include file ' . $file]);
        $oLogger2->expectAt(1, 'logOnly', ['Message from Test_2']);
        $oLogger2->expectAt(2, 'logOnly', ['executed file ' . $file]);
        ...
        $oLogger3->expectOnce('logError', ['Error from Test_3']);
        $oLogger3->expectAt(0, 'logOnly', ['attempting to include file ' . $file]);
        $oLogger3->expectAt(1, 'logOnly', ['executed file ' . $file]);
        ...
        $expected = [
            ...
            'file' => $oUpgrade->upgradePath . 'tasks/openads_upgrade_task_Test_3.php',
            'result' => null,
            'errors' => ['Error from Test_3']];
        $this->assertEqual($result, $expected);
    }

    public function test_createEmptyVarFile()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function test_upgradeSchemasPass()
    {
        $oUpgrade = new OA_Upgrade();
        ...
        $oUpgrade->aDBPackages = [0 => ['version' => '2', 'schema' => 'test_tables', 'files' => '']];
    ...
    public function test_upgradeSchemasFail()
    {
        $oUpgrade = new OA_Upgrade();
    ...
    public function testPutAdmin()
    {
        ...
        // trunkate tables
        DataGenerator::cleanUp(['account_preference_assoc', 'preferences',
    ...
    public function test_rollbackPutAdmin()
    {
        ...
        $this->assertEqual($doPreferencesAssoc->getAll(), []);
        $doPreferences = OA_Dal::factoryDO('preferences');
        $this->assertEqual($doPreferences->getAll(), []);
        $doUserAssoc = OA_Dal::factoryDO('account_user_assoc');
        $this->assertEqual($doUserAssoc->getAll(), []);
        $doUser = OA_Dal::factoryDO('users');
        $this->assertEqual($doUser->getAll(), []);
        $doAgency = OA_Dal::factoryDO('agency');
        $this->assertEqual($doAgency->getAll(), []);
        $doAccount = OA_Dal::factoryDO('accounts');
        $this->assertEqual($doAccount->getAll(), []);
    ...
    public function testCanUpgradeOrInstall()
    {
