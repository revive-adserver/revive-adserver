...
class Test_OA_phpAdsNew extends UnitTestCase
{
    ...
    public function test_migratePANConfig()
    {
        $oPAN = new OA_phpAdsNew();
...
//        $this->assertFalse($aResult[''][''] = $phpAds_config['ignore_hosts'],' incorrect');;
    ...
    public function _putPanConfigFile($fileFrom)
    {
    ...
    public function _setPanConfigFile($aParams)
    {
        $fileTo = MAX_PATH . '/var/config.inc.php';
        if (file_exists($fileTo)) {
            unlink($fileTo);
    ...
    public function _deletePanConfigFile()
    {
        $fileTo = MAX_PATH . '/var/config.inc.php';
    ...
    public function _getMigrationExpectations()
    {
        $i = -1;
