...
while (list(,$element) = each($elements)) {
	...
	if (substr($path, 0, strlen($base) + 8) == $base . '/images/' ||
		substr($path, 0, strlen($base) + 5) == $base . '/css/' ||
		substr($path, 0, strlen($base) + 4) == $base . '/js/') {
		$files[] = $path;
	} else {
...
if (_STRATEGY_ETAG_) {
	...
	if (isset($_SERVER['HTTP_IF_NONE_MATCH']) && get_magic_quotes_gpc()) {
		$_SERVER['HTTP_IF_NONE_MATCH'] = stripslashes($_SERVER['HTTP_IF_NONE_MATCH']);
	}
...
if (_STRATEGY_CACHE_ && file_exists($cache . '/' . $hash)) {
	$contents = file_get_contents($cache . '/' . $hash);
} else {
	$contents = '';
	reset ($files); 
	while (list(,$file) = each($files)) {
