...
class SimulationScenario
{
    ...
    public $oCoreTables;
    public $oDbh;
    public $profileOn = true;
    public $aProfile = [];
    public $adSelectCallback;
    public $aVarDump;
     ...
     */
    public function makeRequests($iteration)
    {
     ...
     */
    public function _init_delivery()
    {
     ...
     *
     */
    public function _simulateCookies()
    {
        //var_dump($_COOKIE);
     ...
     */
    public function _makeRequest($oRequest)
    {
        if ($this->profileOn) {
        ...
        if ($this->profileOn) {
            $this->aProfile[] = ['bannerid' => $adSelect['bannerid'],
     ...
     * @package string $html
     */
    public function _getBeaconURL($bannerId, $html)
    {
        $beaconURL = '';
        if ($bannerId) {
     ...
     * @package string $html
     */
    public function _logBeacon($beaconURL)
    {
        if ($beaconURL) {
     ...
     * @param int $bannerId - if null then delivery failed
     */
    public function _recordDelivery($iteration, $bannerId, $what = '')
    {
        if (!isset($this->aDelivered[$iteration])) {
            $this->aDelivered[$iteration] = [];
        }
        if (!isset($this->aFailed[$iteration])) {
            $this->aFailed[$iteration] = [];
        }
     ...
     * @param int $requests
     */
    public function _summariseInterval($interval, $requests)
    {
     ...
     * @param string $day
     */
    public function setDateTime($hour = 0, $day = '2000-01-01')
    {
     ...
     */
    public function _getDateTimeString()
    {
     ...
     */
    public function _getDateTimestamp()
    {
        $oDate = $this->oServiceLocator->get('now');
     ...
     */
    public function _incDateTimeByOneHour()
    {
        $oSpan = new Date_Span();
        $oSpan->hour = 1;
        $oDate = $this->oServiceLocator->get('now');
     ...
     *
     */
    public function loadRequestset()
    {
    ...
    public function loadDataset($sourceFile = "")
    {
    ...
    public function printResult($query, $title)
    {
     ...
     *
     */
    public function printWorkingData()
    {
     ...
     *
     */
    public function reportResult($result, $msg, $var = '')
    {
        if ($result) {
    ...
    public function printHeading($msg = '', $size = 5)
    {
        if ($msg) {
    ...
    public function printError($message, $data = '')
    {
        $error = true;
        include SIM_TEMPLATES; //MAX_PATH.'/simulation/templates/message.html';
    }

    public function printTable($dbRes, $title = '')
    {
        $printth = true;
        $i = 0;
...
class SimulationRequest
{
    ...
    public function SimulationRequest($what = '', $target = '', $source = '', $referer = '', $loc = '', $ct0 = '', $withText = false, $context = '', $richMedia = true)
    {
        $this->what = $what;
        $this->target = $target;
        $this->source = $source;
