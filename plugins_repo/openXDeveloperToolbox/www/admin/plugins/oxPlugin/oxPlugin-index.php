...
function &buildForm()
{
    ...
    $form = new OA_Admin_UI_Component_Form("oxToolBoxForm", "POST", $_SERVER['SCRIPT_NAME'], null, ["enctype" => "multipart/form-data"]);
    $form->forceClientValidation(true);

    $form->addElement('header', 'header', $oTrans->translate("New Plugin"));

    $form->addElement('text', 'name', $oTrans->translate("Plugin Name"), ['class' => 'medium']);
    ...
    foreach ($aGroups as $extension) {
        $group['name'] = $form->createElement('text', 'group[' . $extension . '][groupname]', $oTrans->translate("Group Name"), ['class' => 'medium']);
        $group['schema'] = $form->createElement('checkbox', 'group[' . $extension . '][schema]', $oTrans->translate("Has Schema"));
        $form->addGroup($group, 'group_' . $extension, $oTrans->translate("Extends %s", [$extension]), "", false);
    }
