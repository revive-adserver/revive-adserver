...
class OaCasClient extends CASClient
{
   ...
   */
        ...
        $cas_response='',$err_code='',$err_msg='')
    {
        ...
        $msgs[] = 'Authentication failure: '.htmlspecialchars($failure);
        if ( $no_response ) {
                ...
                switch ($this->getServerVersion()) {
                    ...
                    case CAS_VERSION_2_0:
                        if ( empty($err_code) )
                            ...
                            $msgs[] = 'Reason: ['.$err_code.'] CAS error: '.htmlspecialchars($err_msg);
                    break;
                }
            }
            $debugMsgs[] = 'CAS response: '.htmlspecialchars($cas_response);
        }
