<?php // $Revision$

/************************************************************************/
/* Openads 2.0                                                          */
/* ===========                                                          */
/*                                                                      */
/* Copyright (c) 2000-2007 by the Openads developers                    */
/* For more information visit: http://www.openads.org                   */
/*                                                                      */
/* This program is free software. You can redistribute it and/or modify */
/* it under the terms of the GNU General Public License as published by */
/* the Free Software Foundation; either version 2 of the License.       */
/************************************************************************/

//  Translator: Tadashi Jokagi <elf2000@users.sourceforge.net>
//  EN-Revision: 2.1.2.4

// Invocation Types
$GLOBALS['strInvocationRemote']			= "Remote Invocation";
$GLOBALS['strInvocationJS']			= "Remote Invocation for Javascript";
$GLOBALS['strInvocationIframes']		= "Remote Invocation for Frames";
$GLOBALS['strInvocationXmlRpc']			= "Remote Invocation using XML-RPC";
$GLOBALS['strInvocationCombined']		= "Combined Remote Invocation";
$GLOBALS['strInvocationPopUp']			= "ポップアップ";
$GLOBALS['strInvocationAdLayer']		= "Interstitial or Floating DHTML";
$GLOBALS['strInvocationLocal']			= "ローカルモード";


// Other
$GLOBALS['strCopyToClipboard']			= "クリップボードに込みーする";


// Measures
$GLOBALS['strAbbrPixels']			= "ピクセル";
$GLOBALS['strAbbrSeconds']			= "秒";


// Common Invocation Parameters
$GLOBALS['strInvocationWhat']			= "バナー選択";
$GLOBALS['strInvocationClientID']		= "広告主かキャンペーン";
$GLOBALS['strInvocationTarget']			= "対象フレーム";
$GLOBALS['strInvocationSource']			= "ソース";
$GLOBALS['strInvocationWithText']		= "バナーの下にテキストを表示する";
$GLOBALS['strInvocationDontShowAgain']		= "Don't show the banner again on the same page";
$GLOBALS['strInvocationDontShowAgainCampaign']		= "Don't show a banner from the same campaign again on the same page";
$GLOBALS['strInvocationTemplate'] 		= "Store the banner inside a variable so it can be used in a template";


// Iframe
$GLOBALS['strIFrameRefreshAfter']		= "Refresh after";
$GLOBALS['strIframeResizeToBanner']		= "Resize iframe to banner dimensions";
$GLOBALS['strIframeMakeTransparent']		= "Make the iframe transparent";
$GLOBALS['strIframeIncludeNetscape4']		= "Include Netscape 4 compatible ilayer";


// PopUp
$GLOBALS['strPopUpStyle']			= "ポップアップの種類";
$GLOBALS['strPopUpStylePopUp']			= "ポップアップ";
$GLOBALS['strPopUpStylePopUnder']		= "Pop-under";
$GLOBALS['strPopUpCreateInstance']		= "Instance when the pop-up is created";
$GLOBALS['strPopUpImmediately']			= "Immediately";
$GLOBALS['strPopUpOnClose']			= "When the page is closed";
$GLOBALS['strPopUpAfterSec']			= "After";
$GLOBALS['strAutoCloseAfter']			= "Automatically close after";
$GLOBALS['strWindowOptions']			= "ウィンドウオプション";
$GLOBALS['strShowToolbars']			= "ツールバー";
$GLOBALS['strShowLocation']			= "場所";
$GLOBALS['strShowMenubar']			= "メニュー";
$GLOBALS['strShowStatus']			= "状態";
$GLOBALS['strWindowResizable']			= "サイズ変更可能";
$GLOBALS['strShowScrollbars']			= "スクロールバー";
$GLOBALS['strPopupInitialHPos']			= "水平位置の初期値";
$GLOBALS['strPopupInitialVPos']			= "垂直位置の初期値";
$GLOBALS['strPopupFromLeft']			= "画面の右側から";
$GLOBALS['strPopupFromRight']			= "画面の左側から";
$GLOBALS['strPopupFromTop']			= "画面の上から";
$GLOBALS['strPopupFromBottom']			= "画面のしたから";
$GLOBALS['strPopupCentered']			= "Centered";


// XML-RPC
$GLOBALS['strXmlRpcLanguage']			= "ホスト言語";


// AdLayer
$GLOBALS['strAdLayerStyle']			= "スタイル";

$GLOBALS['strAlignment']			= "位置あわせ";
$GLOBALS['strHAlignment']			= "水平の位置あわせ";
$GLOBALS['strLeft']				= "左";
$GLOBALS['strCenter']				= "中央";
$GLOBALS['strRight']				= "右";

$GLOBALS['strVAlignment']			= "垂直の位置あわせ";
$GLOBALS['strTop']				= "上";
$GLOBALS['strMiddle']				= "中央";
$GLOBALS['strBottom']				= "下";

$GLOBALS['strAutoCollapseAfter']		= "Automatically collapse after";
$GLOBALS['strCloseText']			= "「閉じる」のテキスト";
$GLOBALS['strClose']				= "[閉じる]";
$GLOBALS['strBannerPadding']			= "Banner padding";

$GLOBALS['strHShift']				= "Horizontal shift";
$GLOBALS['strVShift']				= "Vertical shift";

$GLOBALS['strShowCloseButton']			= "「とjる」ボタンを表示する";
$GLOBALS['strBackgroundColor']			= "背景色";
$GLOBALS['strBorderColor']			= "境界線色";

$GLOBALS['strDirection']			= "方向";
$GLOBALS['strLeftToRight']			= "左から右";
$GLOBALS['strRightToLeft']			= "右から左";
$GLOBALS['strLooping']				= "Looping";
$GLOBALS['strAlwaysActive']			= "Always active";
$GLOBALS['strSpeed']				= "スピード";
$GLOBALS['strPause']				= "一時停止";
$GLOBALS['strLimited']				= "Limited";
$GLOBALS['strLeftMargin']			= "左余白";
$GLOBALS['strRightMargin']			= "右余白";
$GLOBALS['strTransparentBackground']		= "透過背景";

$GLOBALS['strSmoothMovement']		= "Smooth movement";
$GLOBALS['strHideNotMoving']		= "Hide the banner when the cursor is not moving";
$GLOBALS['strHideDelay']			= "Delay before banner is hidden";
$GLOBALS['strHideTransparancy']		= "Transparancy of the hidden banner";


$GLOBALS['strAdLayerStyleName']	= array(
	'geocities'		=> "Geocities",
	'simple'		=> "シンプル",
	'cursor'		=> "カーソル",
	'floater'		=> "Floater"
);

?>